...
public final class JobSettingsAPIImpl implements JobSettingsAPI {
    ...
    public JobSettings getJobSettings(final String jobName) {
        return this.getJobSettings(jobName, "");
    }

    @Override
    public JobSettings getJobSettings(String jobName, String userName) {
        JobSettings result = new JobSettings();
        JobNodePath jobNodePath = new JobNodePath(jobName);
        String configNodePath = jobNodePath.getConfigNodePath();
        if (!StringUtils.isEmpty(userName)) {
            configNodePath = "/" + userName + configNodePath;
        }
        LiteJobConfiguration liteJobConfig = LiteJobConfigurationGsonFactory.fromJson(regCenter.get(configNodePath));
    ...
    @Override
    public void updateJobSettings(JobSettings jobSettings, String userName) {
        Preconditions.checkArgument(!Strings.isNullOrEmpty(jobSettings.getJobName()), "jobName can not be empty.");
        Preconditions.checkArgument(!Strings.isNullOrEmpty(jobSettings.getCron()), "cron can not be empty.");
        Preconditions.checkArgument(jobSettings.getShardingTotalCount() > 0, "shardingTotalCount should larger than zero.");
        JobNodePath jobNodePath = new JobNodePath(jobSettings.getJobName());
        regCenter.update(jobNodePath.getConfigNodePath(userName), LiteJobConfigurationGsonFactory.toJsonForObject(jobSettings));
    }
    ...
    @Override
    public void removeJobSettings(String jobName, String userName) {
        regCenter.remove("/" + userName + "/" + jobName);
    }
}
